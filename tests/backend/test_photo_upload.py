...
def _create_individual(tree_db_client, given="Test", family="Person", sex="M", birth="1980-01-01"):
    r = tree_db_client.post("/individuals", json={
        "sex_code": sex,
        "birth_date": birth,
...
def _upload_photo(tree_db_client, individual_id, age, is_default=False):
...
class TestPhotoUpload:
    ...
    def test_upload_creates_file_with_gedcom_id_and_age(self, tree_db_client, test_owner, log_test_step):
        log_test_step("Upload stores file as <GEDCOM_ID>_<age>.jpg")
        ind = _create_individual(tree_db_client)
        media = _upload_photo(tree_db_client, ind["id"], age=25)
    ...
    def test_upload_collision_appends_suffix(self, tree_db_client, test_owner, log_test_step):
        log_test_step("Second upload at same age gets _2 suffix")
        ind = _create_individual(tree_db_client)
        m1 = _upload_photo(tree_db_client, ind["id"], age=30)
        m2 = _upload_photo(tree_db_client, ind["id"], age=30)
        ...
        m3 = _upload_photo(tree_db_client, ind["id"], age=30)
        assert m3["file_path"] == f"{ind['gedcom_id']}_30_3.jpg"

    def test_upload_rejects_unsupported_type(self, tree_db_client, log_test_step):
    ...
    def test_upload_rejects_nonexistent_individual(self, tree_db_client, log_test_step):
...
class TestSetDefault:
    ...
    def test_set_default_clears_previous(self, tree_db_client, db_utils, log_test_step):
        log_test_step("Upload two photos, first as default")
        ind = _create_individual(tree_db_client)
        m1 = _upload_photo(tree_db_client, ind["id"], age=20, is_default=True)
        m2 = _upload_photo(tree_db_client, ind["id"], age=40, is_default=False)

        assert m1["is_default"] is True

        log_test_step("Set second photo as default via PUT")
        r = tree_db_client.put(f"/media/{m2['id']}/set-default")
        assert r.status_code == 200
        assert r.json()["is_default"] is True

        log_test_step("Verify first photo lost its default status")
        r1 = tree_db_client.get(f"/media/{m1['id']}")
        assert r1.json()["is_default"] is False

    def test_upload_with_default_clears_previous(self, tree_db_client, log_test_step):
        log_test_step("First upload as default")
        ind = _create_individual(tree_db_client)
        m1 = _upload_photo(tree_db_client, ind["id"], age=20, is_default=True)

        log_test_step("Second upload also as default — should clear first")
        m2 = _upload_photo(tree_db_client, ind["id"], age=30, is_default=True)

        r1 = tree_db_client.get(f"/media/{m1['id']}")
        r2 = tree_db_client.get(f"/media/{m2['id']}")
        assert r1.json()["is_default"] is False
...
class TestDeleteMedia:
    ...
    def test_delete_removes_file_on_disk(self, tree_db_client, test_owner, log_test_step):
        ...
        r = tree_db_client.delete(f"/media/{media['id']}")
        assert r.status_code == 200
...
class TestTreePhotoSelection:
    ...
    def test_no_photos_returns_none(self, tree_db_client, log_test_step):
        log_test_step("Individual with no photos -> photo_url=None, photos=[]")
        ind = _create_individual(tree_db_client)
        r = tree_db_client.get(f"/individuals/{ind['id']}/tree?ancestor_depth=0&descendant_depth=0")
        assert r.status_code == 200
        node = r.json()["nodes"][0]
        assert node["photo_url"] is None
        assert node["photos"] == []

    def test_single_photo_used_as_default(self, tree_db_client, log_test_step):
        log_test_step("Single photo (not marked default) -> used as photo_url, promoted in photos list")
        ind = _create_individual(tree_db_client)
        _upload_photo(tree_db_client, ind["id"], age=50)
    ...
    def test_explicit_default_wins(self, tree_db_client, log_test_step):
        log_test_step("Photo marked default is chosen even when another is closer to age 35")
        ind = _create_individual(tree_db_client)
        _upload_photo(tree_db_client, ind["id"], age=34)  # closer to 35
        m2 = _upload_photo(tree_db_client, ind["id"], age=70, is_default=True)
    ...
    def test_closest_to_35_when_no_default(self, tree_db_client, log_test_step):
        log_test_step("No explicit default -> closest to age 35 is chosen")
        ind = _create_individual(tree_db_client)
        _upload_photo(tree_db_client, ind["id"], age=10)
        m2 = _upload_photo(tree_db_client, ind["id"], age=33)  # closest to 35
        _upload_photo(tree_db_client, ind["id"], age=60)
    ...
    def test_no_ages_falls_back_to_first(self, tree_db_client, log_test_step):
        ...
        r1 = tree_db_client.post("/media", json={
            "individual_id": ind["id"],
            "file_path": "fake1.jpg",
            "media_type_code": "photo",
        })
        tree_db_client.post("/media", json={
            "individual_id": ind["id"],
            "file_path": "fake2.jpg",
            "media_type_code": "photo",
        })
    ...
    def test_photos_sorted_by_age(self, tree_db_client, log_test_step):
        log_test_step("Photos in tree response are sorted youngest to oldest")
        ind = _create_individual(tree_db_client)
        _upload_photo(tree_db_client, ind["id"], age=60)
        _upload_photo(tree_db_client, ind["id"], age=10)
        _upload_photo(tree_db_client, ind["id"], age=35)

        r = tree_db_client.get(f"/individuals/{ind['id']}/tree?ancestor_depth=0&descendant_depth=0")
        ages = [p["age"] for p in r.json()["nodes"][0]["photos"]]
        assert ages == [10, 35, 60]

    def test_equidistant_ages_picks_one(self, tree_db_client, log_test_step):
        log_test_step("Age 30 and age 40 are equidistant from 35 — one should be promoted")
        ind = _create_individual(tree_db_client)
        _upload_photo(tree_db_client, ind["id"], age=30)
        _upload_photo(tree_db_client, ind["id"], age=40)
...
class TestServeFile:
    ...
    def test_serve_uploaded_photo(self, tree_db_client, log_test_step):
        log_test_step("Upload then fetch — should return image/jpeg")
        ind = _create_individual(tree_db_client)
        media = _upload_photo(tree_db_client, ind["id"], age=25)

        r = tree_db_client.get(f"/media/{media['id']}/file")
        assert r.status_code == 200
        assert r.headers["content-type"].startswith("image/jpeg")
        assert len(r.content) > 0

    def test_serve_nonexistent_returns_404(self, tree_db_client, log_test_step):
        log_test_step("Non-existent media id returns 404")
        r = tree_db_client.get("/media/99999/file")
        assert r.status_code == 404
